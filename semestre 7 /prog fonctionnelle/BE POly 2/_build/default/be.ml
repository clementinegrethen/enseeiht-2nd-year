...
let evalue = fun l x ->   
  (List.fold_left((fun acc e ->(e+.acc)*.x)) 0. (List.tl l)) +. List.hd l  ;;
...
let retract = fun poly  ->  
  let  l = ref [] and i = ref ((List.length poly)-1) in
      while (List.nth poly !i) = 0. do
        i := !i-1
      done;
      for j = 0 to !i do
        l := (List.nth poly j)::!l
...
let scal_mult = fun x poly -> List.map(fun e -> e*.x) poly;;
...
(*let plus  = fun poly1 poly2 -> 
  let i = List.length poly1 and j=List.length poly2 and l= ref [] and p = ref 0  in
  if i<=j then p:=i else p:=j;
  for k=0 to !p do 
    l:=!l@[ (List.nth poly1 k) + (List.nth poly2 k)]
  done;
   l:=!l@ (List.drop p poly1)@ (List.drop p poly2);
   !l;;*)
...
*)
let decoder  = fun code arbre1-> 
  let l = ref [] in
  let rec aux = fun arbre liste_code -> 
    match arbre,liste_code with
    | Vide,_ -> raise CodeNonValide
    |(Lettre c, []) -> l:=!l@[c]
    | Lettre c,li-> l:=!l@[c]; aux arbre1 li
    | Noeud (a1,a2,a3),h::t -> if h=1 then aux a1 t else if h=2 then aux a2 t else if h=3 then aux a3 t else raise CodeNonValide
    | Noeud (a1,a2,a3),[] -> raise CodeNonValide
  in
  aux arbre1 (decompose_int code) ;
  recompose_chaine !l;;
...
*)
let arbre_to_liste = fun a -> 
  let l = ref [] in
  let rec aux = fun arbre code -> 
    match arbre with
    | Vide -> ()
    | Lettre c -> l:=!l@[(c,code)]
    | Noeud (a1,a2,a3) -> aux a1 (code*10+1); aux a2 (code*10+2); aux a3 (code*10+3)  
  in
  aux a 0;
  !l;;
...
*)
let encoder = fun arbre mot ->
  let lis=ref [] in
  let  rec encoder_char e  l  =
    match l with 
    |[]-> failwith"wesh"
    |(lettre,code)::q when lettre=e -> code
    |_::q -> encoder_char e q  in 
    lis:=List.map (fun e-> (encoder_char e (arbre_to_liste arbre))) (decompose_chaine mot);
    recompose_int !lis;;
