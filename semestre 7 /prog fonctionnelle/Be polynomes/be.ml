...
let evalue = fun x0 p ->
(( List.hd p) +. (List.fold_left (fun acc e -> e*.x0+.acc) 0. (List.tl p)) );;
...
let scal_mult = fun x poly ->
  List.map (fun e -> e*.x) poly;;
...
let plus  = fun poly1 poly2 ->
  let n1=List.length poly1 and n2=List.length poly2 and  l=ref [] in
  for i =0 to (min n1 n2)-1 do 
    l:=((List.nth poly1 i)+.(List.nth poly2 i ))::(!l)
  done;
    if n1>n2 then
    for i =n2 to n1-1 do 
      l:=((List.nth poly1 i))::(!l)
    done
  else
    for i =n1 to n2-1 do 
      l:=((List.nth poly2 i))::(!l)
    done;
  l:=retract (List.rev !l);
  !l;;
...
*)
let decoder  = fun code arbre ->
  let code_liste = decompose_chaine(string_of_int code )in 
  let rec aux = fun code_liste arbre_aux acc ->
    match code_liste,arbre_aux with
    |[],Lettre c -> c::acc
    |l, Lettre c-> aux l arbre (c::acc)
    |t::q, Noeud (a,b,c) -> if t='1' then aux q a acc else if t='2' then aux q b acc else  aux q c acc 
    
  in
  List.rev (aux code_liste arbre []);;
