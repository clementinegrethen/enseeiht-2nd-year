  ...
  *)
  ...
  | FunctionToken -> (print_endline "function ident fleche E");
    (match (accept FunctionToken stream) with
      | Success stream -> (match (acceptIdent stream) with
        | Success stream -> (match (accept BodyToken stream) with
          | Success stream -> (parseE stream)
          | Failure -> Failure)
        | Failure -> Failure)
      | Failure -> Failure)
...
and parseFX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | Success next -> (match (parseF next) with
      ...
      | _ -> Failure)
    | _ -> Failure )
    (* regle #15 - FX ->          | { $, +, -, =, ),then,else } *)
    | EOSToken | PlusToken | MinusToken | EqualToken | RightParenthesisToken|ElseToken|ThenToken -> (print_endline "");(Success stream)
    | _ -> Failure)
