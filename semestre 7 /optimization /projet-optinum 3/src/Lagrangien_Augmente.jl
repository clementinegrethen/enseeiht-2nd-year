...
function Lagrangien_Augmente(algo,fonc::Function,contrainte::Function,gradfonc::Function,
  ...
  while nb_iter < itermax && !CN1 && !stagnation_xk && !stagnation_f
    ...
    if algo == "newton"
      new_x  = Algorithme_De_Newton(La,grad_LA,hess_LA,x,[])[1]
    else
      new_x = Regions_De_Confiance(algo,La,grad_LA,hess_LA,x,[])[1]
    end 

    if norm(contrainte(new_x)) >= etatk
      muk = tho*muk
      epsk = eps0/muk
      push!(muks, muk)
      etatk = etat/(muk^alpha)
    else 
      lambdak = lambdak + muk*contrainte(new_x)
      epsk = epsk/muk
      push!(lambdaks,lambdak)      
      etatk = etatk/(muk^beta)
    end

    #Mise à jour des conditions d'arrêts

    stagnation_xk = (norm(new_x - x) <= epsk)
    stagnation_f = (abs(fonc(new_x) - fonc(x)) <= epsk)
    CN1 = ( norm(La(new_x)) <= epsk ) 
    x = new_x
    nb_iter = nb_iter + 1
