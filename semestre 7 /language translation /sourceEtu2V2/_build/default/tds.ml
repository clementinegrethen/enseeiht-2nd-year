...
type info =
  ...
  (* Infos nécessaire pour  pour traiter les boucles à la rust*)
...
(* Permet de retourner les info_ast (sous forme d'une liste) correspondant à la fonction nom *)
      let rec chercherGlobalementFonctions tds nom =
        match tds with
        | Nulle -> []
        | Courante(tm, tc) ->
          (* On commence par rechercher les fonctions dans la table courante *)
          let fonctions =
            try
              (* On vérifie si la fonction est dans la table courante *)
              let info = Hashtbl.find tc nom in
              match !info with
              | InfoFun(_, _, _) -> [info]
              | _ -> [info]
            with
              Not_found -> []
          in
          (* On ajoute les fonctions trouvées dans la table courante à la liste des résultats *)
          let resultat = fonctions @ (chercherGlobalementFonctions tm nom) in
          resultat
