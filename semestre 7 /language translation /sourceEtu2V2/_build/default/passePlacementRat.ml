...
let rec analyse_placement_instr i depl reg =
  match i with
    ...
    |AstType.Loop(b) -> let nb = analyse_placement_bloc b depl reg in
    ...
    | AstType.SwitchCase(_,l) -> let nl = List.map (fun (e,b) -> let nb = analyse_placement_bloc b depl reg in (e,nb)) l in
      (AstPlacement.SwitchCase(nl), 0)
