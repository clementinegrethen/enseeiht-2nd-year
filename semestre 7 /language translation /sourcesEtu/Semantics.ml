...
and ruleName env name =
  ...
  | Found value -> match value with  
    | FrozenValue (expr, env) -> value_of_expr expr env
    | FixPoint (expr, env) -> value_of_expr expr env
    | _ -> value
...
(* Fonction d'évaluation d'un letrec *)
...
and ruleFunction expr env =
  match expr with
  | FunctionNode (ident, body) -> FrozenValue (expr, env)
  | _ -> ErrorValue UndefinedExpressionError
...
and ruleCallByName env fexpr pexpr =
  ...
  | FrozenValue (FunctionNode (ident, body), fenv) ->
    ...
    let newenv = (ident, pvalue) :: fenv in
    value_of_expr body newenv
  | _ -> ErrorValue UndefinedExpressionError
