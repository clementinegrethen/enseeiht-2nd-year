...
public class Rasterizer {
	...
	int numAttributes = v1.getNumAttributes ();
        ...
        // Si la différence en y est plus grande que la différence en x), alors les coordonnées x et y des points de la ligne sont échangées
        ...
        //si x1 est plus grand que x2, les points de la ligne sont échangés, 
        //ce qui garantit que la ligne est toujours tracée de gauche à droite.
        if (x1 > x2) {
            Fragment ftemp;
            int temp;
        ...
        // savoir si la ligne est tracée de haut en bas ou de bas en haut
        int ystep;
        if (y1 < y2) {
            ystep =  1;
        } else {
		...
		// Point de départ de l'algorithme de Bresenham
		int x = x1;
		float y_courant = y1;
		int y=y1;
		float delta_y = y2-y1;
		...
		for (int i=1;i<=delta_x;i++) {
            // on avance d'un pixel en x et on calcule y
			x = x+1; 
			y_courant = y_courant + m;
            // on arrondit y: y_courant représente la valeur idélale de y: mais on ne peut pas toujours avoir un pixel à y_courant (grille discrète)
            // Donc on arrondit y_courant à l'entier le plus proche
			if ((ystep == 1)&&(y_courant < y+0.5)||((ystep == -1) && (y_courant > y -0.5))) {
                // si y_courant est suffisamment proche de y,alors y reste le même
				y = y;
			} else {
                // on arrondit y_courant à l'entier le plus proche
				y = y + ystep;
			}	
    ...
    public void rasterizeFace (Fragment v1, Fragment v2, Fragment v3) {
       ...
       int x3 = v3.getX ();
       int y3 = v3.getY ();
       // boîte englobante du triangle: le plus petit rectangle qui contient le triangle.
